    ...
    IPermissionService permissionService) : IRequestHandler<DeleteAccountCommand, bool>
{
    public async Task<bool> Handle(DeleteAccountCommand request, CancellationToken cancellationToken)
    {
        ...
        if (account == null) 
            throw new KeyNotFoundException("Invalid Account or Account already inactive");

        if (account.System && !permissionService.IsAdmin())
            throw new UnauthorizedAccessException("Cannot delete a system account without administrator privileges");
